...
public readonly record struct DateRange : IComparable<DateRange>
{
    ...
    public DateRange(DateOnly startDate, DateOnly endDate)
    {
        if (endDate < startDate || startDate == default || endDate == default)
        {
            throw new ArgumentException(
                "Invalid date range: End date cannot be before start date and dates cannot be default values.");
        }
    ...
    /// </summary>
    /// <returns>The total number of days in the date range.</returns>
    public int Duration()
    {
    ...
    /// <returns>The intersected date range, or an empty range if there is no intersection.</returns>
    ...
    /// </summary>
    /// <returns>An enumerable of <see cref="DateOnly" /> representing each day in the range.</returns>
